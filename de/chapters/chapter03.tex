...
\end{quote}
Heute würden wir dies als quadratische Gleichung\index{Quadratische Gleichung}
\[
x^2 + 10x = 39
\]
schreiben.  

Al-Chwarizmi dachte jedoch geometrisch\index{Geometrie}: 
Das Glied \(x^2\) stellte ein Quadrat dar, dessen Seitenlänge \(x\) ist. 
Die Terme \(10x\) bildeten zwei Rechtecke der Breite~5, die an zwei Seiten des Quadrats anliegen.  
Das fehlende kleine Quadrat in der Ecke, das die Figur zu einem größeren Quadrat ergänzt, 
hat die Fläche \(5^2 = 25\).  
Durch Hinzufügen dieser Fläche wird das Gebilde zu einem vollständigen Quadrat\index{Quadratische Ergänzung}:
\[
x^2 + 10x + 25 = 39 + 25,
\]
...
\SI{3}{\liter}. Zu Beginn enthält er \SI{20}{\liter}.  
...
(siehe Abschnitt~\ref{sec:3.3_entstehung}).
...
\subsubsection*{Einleitung}
\phantomsection
Im 16.~Jahrhundert\index{16. Jahrhundert} arbeiteten italienische Mathematiker intensiv an der
Lösung kubischer Gleichungen\index{Kubische Gleichung}. Dabei tauchte ein Ausdruck auf, 
...
kubischer Gleichungen. Bei bestimmten Gleichungstypen — insbesondere im sogenannten
\emph{casus irreducibilis}\index{Casus irreducibilis} — entstanden Zwischenausdrücke wie
\[
\sqrt{-121}.
\]
...
(siehe Abschnitt~\ref{sec:3.4_regelsystem}).
